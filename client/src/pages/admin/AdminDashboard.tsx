    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-50 pb-12 dark:bg-slate-950">
      ...
      <header className="relative flex items-start justify-between px-6 pb-7 pt-12">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-900 dark:text-slate-50">Tableau de bord</h1>
        </div>
        <Navigation />
      </header>
      ...
      <main className="relative mx-auto w-full max-w-2xl px-6">
        {loading ? (
          <p className="py-20 text-center text-sm text-slate-400">Chargement…</p>
        ) : (
          <>
            ...
            >
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-100">Statuts d'inscription</span>
              </div>
              ...
              <div className="mt-2 flex justify-center gap-5">
                {statutData.map((d) => (
                  <span key={d.name} className="flex items-center gap-1.5 text-xs font-medium text-slate-400">
            ...
            >
              <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-100">Top 5 commerces les plus scannés</span>
              </div>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={topCommerces} layout="vertical" margin={{ top: 0, right: 12, left: 8, bottom: 0 }}>
                  ...
                  <YAxis
                    ...
                    width={92}
                  />
                  <Tooltip cursor={{ fill: 'rgba(124,58,237,0.08)' }} contentStyle={tooltipStyle} labelStyle={{ color: '#a78bfa', fontWeight: 700 }} />
                  <Bar dataKey="scans" name="Scans" fill="#7C3AED" radius={[0, 6, 6, 0]} barSize={18} />
                </BarChart>
              </ResponsiveContainer>
            </motion.div>
          </>
